...
const SIDE_PROJECTS = [
  ...
  },
  {
    ...
    desc: "AI assistant deployed for a personal injury law firm on its own domain. One conversational assistant streams answers from live case data — CASEpeer, Dropbox, Gmail/Calendar, client texts, and call logs — leads every morning brief with Statute of Limitations deadlines, and only acts (notes, tasks, document review) after explicit approval. Also live in Microsoft Teams as an @mention bot with its own short-term memory.",
    built: [
      "React / Vite frontend on Vercel + FastAPI backend on Render, live on a custom domain",
      "Unified chat: single conversational thread, SSE token streaming, live tool-status updates",
      "CASEpeer + Dropbox + Kenect (SMS) + RingCentral (calls) all reconciled into one Postgres store via Zapier",
      "Statute of Limitations tracking — the brief leads with SOL deadlines across all 227 imported cases",
      ...
      "Microsoft Teams bot (Bot Framework, single-tenant) — live @mention/DM Q&A with short per-conversation memory, plus an automated daily Adaptive Card brief",
    ],
    next: "Two-way SMS replies · client intake flow",
    stack: "React · Vite · FastAPI · PostgreSQL · Claude Sonnet · MCP · SSE · Render · Vercel · Zapier · Google OAuth · Microsoft Teams · Kenect · RingCentral",
