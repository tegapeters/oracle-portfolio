...
const SIDE_PROJECTS = [
  {
    ...
    title: "Job Pal",
    desc: "Agentic job search pipeline. Scrapes tech and academic job sources in parallel, scores every listing against an uploaded resume with a 3-pass pipeline (heuristic → XGBoost → Claude Sonnet), generates tailored cover letters for strong matches, and surfaces them in a branded Streamlit dashboard.",
    built: [
      "4 tech sources (LinkedIn, Adzuna, USAJobs, Google Jobs) + 2 academic verticals in parallel",
      "Three-pass scoring: heuristic pre-filter → XGBoost binary classifier → Claude Sonnet 1–10",
      "Cover letter generation for 8+ matches with resume-grounded specifics",
      "Faculty / adjunct vertical — InsideHigherEd + HigherEdJobs with 7 category feeds",
      "Gmail IMAP rejection scanner — batched OR search, confirmation email filtering",
    ...
    stack: "Python · Claude Sonnet · XGBoost · Supabase · Streamlit",
  ...
  },
  {
    ...
    title: "Eshie — AI Legal Ops Assistant",
    desc: "AI assistant deployed for a personal injury law firm. One conversational assistant streams answers from live case data — case management, documents, Gmail/Calendar, client texts, and call logs — leads every morning brief with Statute of Limitations deadlines, and only acts (notes, tasks, document review) after explicit approval. Also live in Microsoft Teams as an @mention bot with its own short-term memory.",
    built: [
      "Full-stack cloud deployment: React / Vite frontend + FastAPI backend, live on a custom domain",
      "Unified chat: single conversational thread, SSE token streaming, live tool-status updates",
      "Case management, document, SMS, and call systems reconciled into one Postgres store via Zapier",
      "Statute of Limitations tracking — the brief leads with SOL deadlines across the full active case library",
      "Human-in-the-loop approve-to-act: case notes, task creation, and document-review clearing all gated behind explicit approval",
      "Microsoft Teams bot (single-tenant) — live @mention/DM Q&A with short per-conversation memory, plus an automated daily Adaptive Card brief",
    ],
    next: "Two-way SMS replies · client intake flow",
    stack: "React · Vite · FastAPI · PostgreSQL · Claude Sonnet · MCP · SSE · Cloud deployment · Zapier · Google OAuth · Microsoft Teams",
