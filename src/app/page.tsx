...
const SIDE_PROJECTS = [
  ...
  },
  {
    ...
    desc: "AI assistant deployed for a personal injury law firm on its own domain. Surfaces case status, client messages, and a personalized morning brief — shared case data from CASEpeer and Dropbox, plus each employee's own Gmail and Google Calendar — without staff opening the case management system. The firm-wide brief also auto-posts to Microsoft Teams every weekday morning.",
    built: [
      "React / Vite frontend on Vercel + FastAPI backend on Render, live on a custom domain",
      "Custom MCP server exposing case-management tools — staff log notes and create tasks through chat",
      "CASEpeer + Dropbox sync via Zapier (4 live Zaps) — new cases, status changes, leads, and documents",
      "Google OAuth sign-in with per-user Gmail + Calendar tool-calling for a personalized inbox brief",
      "Automated daily brief posted to Microsoft Teams as an Adaptive Card (scheduled webhook, weekday mornings)",
      "Interactive Teams bot for @mention/DM case Q&A — built on the Bot Framework, code-complete and pending activation",
    ],
    next: "Turn on the Teams bot · Kenect SMS webhook · RingCentral calls",
    stack: "React · Vite · FastAPI · PostgreSQL · Claude Sonnet · MCP · Render · Vercel · Zapier · Google OAuth · Microsoft Teams",
